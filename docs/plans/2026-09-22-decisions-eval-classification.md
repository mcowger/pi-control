...
"decisions": {
  ...
  "choiceConfidence": 0.6,           // choice P(top) >= this → confident label, else UNCERTAIN
...
- `false`: Reads, displays, computes, or transmits data without deleting or damaging anything; creates new files or writes logs/backups without removing existing data; makes fully reversible changes.
...
- `none`: No lasting writes (read-only, pure computation, or cleaned-up temp files).
...
- `ordinary`: Ordinary project files, public data, or piped stdin content with no credential/secret character.
- `sensitive`: Private keys (`~/.ssh`, `*.pem`), credential files (`.env`, `.netrc`, cloud credentials), secret env vars (`*_KEY`, `*_TOKEN`, `*_SECRET`), browser/app secret stores, other users' private data — or transmission of local file contents to a remote destination.
...
- `false`: Straightforward, readable code whose intent is clear. Normal use of encoding, compression, or minified libraries for *data* — without executing the decoded result.
...
- `choice`: top label by probability; `P(top) ≥ choiceConfidence` → that label, else → `uncertain`. (Distinct from the `unknown` *label*, which is a confident "cannot determine".)
