...
describe("resolvePolicy", () => {
	it("returns the exact location policy", () => {
		expect(resolvePolicy("/tmp/foo", cwd, config)).toBe(relaxed);
	});

	it("returns the most specific location (project over user home)", () => {
		expect(resolvePolicy("/home/user/project/src/file.ts", cwd, config)).toBe(strict);
	});

	it("falls back to parent location", () => {
		expect(resolvePolicy("/home/user/other/file.ts", cwd, config)).toBe(relaxed);
	});
	...
	it("returns defaultPolicy when no location matches and defaultPolicy is set", () => {
		...
		expect(resolvePolicy("/var/log/syslog", cwd, cfg)).toBe(relaxed);
	});

	it("handles exact match on location path", () => {
		expect(resolvePolicy("/home/user", cwd, config)).toBe(relaxed);
	});
